...
module.exports = {
    name: "use",
    aliases: ["useitem", "itemuse"],
    permissions: [],
    description: "This is the use command. Here, you can use useable items!",
    usage: "Type \`{prefix} use [item] [amount (optional)]\`.",
    cooldown: 4,
    cooldownMsg: 'If you use items too much, you\'ll... you\'ll... oh I don\'t know! There\'s a cooldown alright, so',
    async execute(client, message, cmd, args, Discord, profileModel, profileData, cooldownCheck, additionalInput) {
		if (args.length === 0) { message.reply('how do you use an item that doesn\'t have a name'); return cooldownCheck.cancel() }
		...
		if (!item.use) { message.reply('You can\'t use this item 🤔'); return cooldownCheck.cancel() }
        if (profileData.activeItems.map(e => e.name).includes(item.name)) { message.reply(`This item is active, so you can\'t use it!`); return cooldownCheck.cancel() }
        ...
        if(time_stamps.has(message.author.id)) {
            ...
            if(current_time < expiration_time){
                ...
                .setColor('BLACK')
                .setDescription(`You can use this item <t:${Math.floor(expiration_time/1000)}:R>`)
        ...
        if (!changes.loss && !changes.stopCooldown && !item.eat) {
